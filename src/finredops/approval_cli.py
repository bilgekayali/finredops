"""v0.7.1 operator commands for key-backed risk and report approvals."""
...
from .approval_trust import (
    ...
    approval_signature_signing_document,
...
from .reporting import report_from_document
...
def approval_help() -> str:
    return (
        ...
        "  risk-acceptance-signature-request create exact risk-owner signing request\n"
        "  report-approval-signature-request create exact report-approver signing request\n"
        "  finalize-approval-signature       attach externally produced Ed25519 signature\n"
...
def _parser(command: str) -> argparse.ArgumentParser:
    ...
    common_signer = argparse.ArgumentParser(add_help=False)
    common_signer.add_argument("--engagement-id", required=True)
    common_signer.add_argument("--issuer", required=True)
    common_signer.add_argument("--key-id", required=True)
    common_signer.add_argument("--issued-at", required=True)
    common_signer.add_argument("--expires-at", required=True)
    common_signer.add_argument("--output", type=Path, required=True)

    if command == "risk-acceptance-signature-request":
        parser = argparse.ArgumentParser(prog=f"finredops {command}", parents=[common_signer])
    ...
    if command == "approve-trusted-report":
        ...
        return parser
    raise ValueError(f"Unknown approval command: {command}")
...
def _validated_resolution(path: Path, engagement_id: str) -> Mapping[str, Any]:
    document = read_review_json(path)
    if not isinstance(document, Mapping):
        raise ApprovalTrustError("Trust resolution must be a JSON object.")
    required = {
        "schema_version",
        "engagement_id",
        "trust_bundle_digest",
    ...
    if set(document) != required or document.get("schema_version") != "finredops.trusted-review-resolution.v1":
        raise ApprovalTrustError("Trust resolution does not match the v1 contract.")
    body = {key: document[key] for key in document if key != "resolution_digest"}
    if sha256_digest(body) != document.get("resolution_digest"):
        raise ApprovalTrustError("Trust resolution digest is invalid.")
    if document.get("engagement_id") != engagement_id or document.get("cryptographic_signatures_verified") is not True:
        raise ApprovalTrustError("Trust resolution does not match the requested engagement or trust state.")
    return document


def _validated_trusted_manifest(path: Path, engagement_id: str) -> Mapping[str, Any]:
    document = read_review_json(path)
    if not isinstance(document, Mapping):
        raise ApprovalTrustError("Trusted-promotion manifest must be a JSON object.")
    ...
    supplied = document.get("trusted_promotion_digest")
    body = {key: document[key] for key in document if key != "trusted_promotion_digest"}
    if not isinstance(supplied, str) or sha256_digest(body) != supplied:
...
def _risk_request(args: argparse.Namespace) -> dict[str, Any]:
    ...
    resolution = _validated_resolution(args.trust_resolution, args.engagement_id)
    ...
    if not isinstance(authoritative, list) or review.review_id not in authoritative:
        raise ApprovalTrustError("Risk acceptance is not bound to a current authoritative review.")
    return approval_signature_signing_document(
        {
            "issuer": args.issuer,
...
def _report_request(args: argparse.Namespace) -> dict[str, Any]:
    ...
    manifest = _validated_trusted_manifest(args.trusted_promotion_manifest, args.engagement_id)
...
def _verify_risk_acceptances(args: argparse.Namespace) -> dict[str, Any]:
    batch = read_intake_file(args.intake)
    ...
    if not isinstance(authoritative, list) or set(authoritative) != {item.review_id for item in reviews}:
        raise ApprovalTrustError("Supplied reviews must exactly match the trust resolution authority set.")
    acceptances, signed = load_verified_risk_acceptances(
        ...
        engagement_id=args.engagement_id,
        trust_resolution_digest=str(resolution["resolution_digest"]),
...
def run_approval_command(argv: Sequence[str]) -> int:
    command = argv[0]
    ...
    try:
        if command == "risk-acceptance-signature-request":
            ...
            print(json.dumps({"signature_id": request["signature_id"], "output": str(args.output), "signature_required": True}, indent=2))
            return 0
        if command == "report-approval-signature-request":
        ...
        if command == "finalize-approval-signature":
            ...
            if not isinstance(request, Mapping):
                raise ApprovalTrustError("Approval signing request must be an object.")
            ...
            return 0
        if command == "verify-signed-risk-acceptances":
            print(json.dumps(_verify_risk_acceptances(args), ensure_ascii=False, indent=2))
            return 0
        if command == "approve-trusted-report":
            ...
            return 0
        raise ApprovalTrustError("Unknown approval command.")
    except (OSError, ValueError, ApprovalTrustError) as exc:
        print(f"INVALID: {exc}")
        return 1

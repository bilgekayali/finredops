...
def _change_material(args: argparse.Namespace):
...
def _parser() -> argparse.ArgumentParser:
    ...
    _add_change_control_args(register)
    ...
    _add_change_control_args(disable)
...
def run_postgres_command(argv: Sequence[str]) -> int:
    ...
    try:
        ...
        if args.command == "postgres-service-account-sql":
            bundle, package = _change_material(args)
            ...
            verify_postgres_mapping_change_package(mapping, package, bundle)
            ...
            print(f"Approved PostgreSQL service-account mapping SQL: {args.output}")
            return 0
        ...
        if args.command == "postgres-disable-service-account-sql":
            ...
            print(f"Approved PostgreSQL service-account disable SQL: {args.output}")
            return 0
...
PostgreSQL RLS / service-account commands:
  ...
  postgres-service-account-sql        Write independently approved LOGIN-role mapping SQL
  postgres-disable-service-account-sql Write independently approved mapping-disable SQL
